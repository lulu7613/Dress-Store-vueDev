  ...
  <main class="my-5">
    ...
    <div class="container justify-content-center">
      <div class="form-row mt-5">
        ...
        <div class="col-12 col-sm">
          <div
            class="alert text-center alert-success alert-radius"
            :class="{'alert-light': order.is_paid}"
            style="border-radius: 50px"
            role="alert"
          >2. 金流付款</div>
        </div>
    ...
    <div class="row justify-content-center my-4" v-if="order.is_paid">
      <div class="col-md-6">
    ...
    <div class="my-5 pb-4 row justify-content-center">
...
export default {
  ...
  methods: {
    ...
    payOrder () {
      const vm = this
      vm.isLoading = true
      const api = `${process.env.API_PATH}/api/${process.env.API_ADMIN}/pay/${vm.order.id}`
      this.$http.post(api, vm.order.id).then((response) => {
        console.log('結帳-付款完成', response.data)
  ...
  created () {
    ...
    const api = `${process.env.API_PATH}/api/${process.env.API_ADMIN}/order/${orderId}`
    this.$http.get(api).then((response) => {
      console.log('結帳-取得訂單', response.data)
